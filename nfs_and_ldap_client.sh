...
nslcd nslcd/ldap-uris string ldap://ldapserver
...
ldap-auth-config ldap-auth-config/ldapns/ldap-server string ldap://ldapserver/
...
sed -i 's,uri ldapi:///,uri ldap://ldapserver,g' /etc/ldap.conf #can use something different from "/" as seperator
