...
nslcd nslcd/ldap-uris string ldap://ldap-server007
...
ldap-auth-config ldap-auth-config/ldapns/ldap-server string ldap://ldap-server007/
...
sed -i 's,uri ldapi:///,uri ldap://ldap-server007,g' /etc/ldap.conf #can use something different from "/" as seperator
...
echo "nfs-server:/var/nfsshare/testing  /mnt/test nfs defaults 0 0" >> /etc/fstab
